...
class ScanByFingerActivity1 : BaseActivity(), View.OnClickListener, ImageListener, VitalSignsListener, SessionInfoListener {
                ...
                when (it.status) {
                    ...
                    Status.SUCCESS -> {
                        ...
                        if (it.data != null && it.code == ResponseStatus.STATUS_CODE_SUCCESS) {
                            CommonUtils.showToast(this@ScanByFingerActivity1, it.data.message)
                            fingerResultID = it.data.id
                            MeasurementResultActivity.startActivity(
                                this@ScanByFingerActivity1, fingerResultID
                            )
                            finish()
                            AnimationsHandler.playActivityAnimation(
                                this@ScanByFingerActivity1, AnimationsHandler.Animations.RightToLeft
                            )
                        }
                    }
                    ...
                    Status.ERROR -> {
                        showHideProgress(false)
                        CommonUtils.showToast(this@ScanByFingerActivity1, it.message)
                    }
                }
            }
        }
...
//        val subjectDemographic = SubjectDemographic(Sex.FEMALE, 35.0, 65.0)
        try {
            ...
            session = FingerSessionBuilder(applicationContext)
        ...
        try {
            ...
            when (e.errorCode) {
                AlertCodes.DEVICE_CODE_MINIMUM_BATTERY_LEVEL_ERROR -> {
                    ...
                    AlertDialogManager.showInformationDialog(this@ScanByFingerActivity1,
                        0,
...
//            if (mLicenseEnabledVitalSigns.isHeartRateEnabled) "--" else getString(R.string.na)
    ...
    private fun updateReadingProgressMsg() {
        if (isDetected) {
            when (mTime) {
                in 0..5 -> {
                    binding.measurementsLayout.tvScanningMsg.text = getString(R.string.finger_scanning_msg)
                }
                ...
                in 7..12 -> {
                    binding.measurementsLayout.tvScanningMsg.text = getString(R.string.symptoms_of_hypoxemia_include_wheezing)
                }

                in 13..18 -> {
                    binding.measurementsLayout.tvScanningMsg.text = getString(R.string.prq_is_calculate_by_dividing_heart_rate)
                }
                ...
                in 31..36 -> {
                    binding.measurementsLayout.tvScanningMsg.text = getString(R.string.make_sure_that_your_finger_completely_covers_the_camera)
                }

                in 37..42 -> {
                    binding.measurementsLayout.tvScanningMsg.text = getString(R.string.high_blood_pressure_may_exist_wth_no_noticeable)
                }

                in 43..48 -> {
                    binding.measurementsLayout.tvScanningMsg.text = getString(R.string.normal_prq_is_4_to_5)
                }
...
//                }
                ...
                binding.measurementsLayout.tvReading.visibility = View.GONE
                binding.measurementsLayout.tvReadingUnit.visibility = View.GONE
                binding.measurementsLayout.readingProgressBar.setProgressDrawableColor(ContextCompat.getColor(this, R.color.error_msg_text_color))
            }
        }
    }
...
//                    CommonUtils.showToast(this, getString(R.string.no_result_found))
                    Log.d(ScanByFingerActivity1.tag, "No result calculated")
                } else {
    ...
    override fun onLicenseInfo(licenseInfo: LicenseInfo) {
        runOnUiThread {
            ...
            licenseInfo.licenseOfflineMeasurements?.let { offlineMeasurements ->
                Log.i(tag, "License Offline Measurements: " + "${offlineMeasurements.totalMeasurements}/" + "${offlineMeasurements.remainingMeasurements}")
            }
        }
    }
